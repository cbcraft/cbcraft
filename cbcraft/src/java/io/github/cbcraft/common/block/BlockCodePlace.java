	...
	public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumFacing side, float hitX, float hitY, float hitZ) {
		...
		if(!worldIn.isRemote) {
			if(playerIn.inventory.getCurrentItem() == null) {
				Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("Place Paramter: Place direction is set to '" + tileEntityCodePlace.getBlockParamter("direction") + "'"));
				if(tileEntityCodePlace.getBlockParamter("block") == "minecraft:air") {
					Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText("Place Paramter: Place block is not set"));
				}
				else {
